"""Auto Farmland v2.0.0.0 plugin for Amulet Map Editor.

Purpose:
- Scan the selected Minecraft Bedrock Edition area for safe exposed surfaces.
- Replace grass blocks with farmland or build a conservative raised farm layer.
- Plant fixed, alternating, assorted or stem-based crop layouts.
- Detect irrigation, plan additional water and optionally cover new water.
...
Navigation:
1. Bedrock block references, crop mappings and safety rules
2. Green custom UI theme and shared helpers
3. Local crop and slab artwork from Amulet's resource cache
4. Painted controls, scrolling, selectors and dialogs
5. Layered Windows dropdowns and tooltips with portable fallbacks
6. Floating window, launcher and window lifecycle
7. Settings persistence and plugin-file management
8. Reports, selection handling and surface scanning
9. Crop layout, hydration and water planning
10. Plan validation, world writing and Amulet export
"""
...
# Creative-accessible Bedrock upper slabs represented by the explicit
# mapping below. Every entry uses the same vertical-half state, while the
# display-name mapping avoids guessing block identifiers.
SLAB_BLOCK_NAMES = {
...
# Most Bedrock slab identifiers match the local Java item-model names used by
# Amulet's resource cache. The normal stone slab is the one required alias.
SLAB_ICON_ITEM_IDS = {
    display_name: (
...
# =========================
# Auto Farmland uses the same self-themed UI contract as Auto Light.
# Its passive window and card tint is a restrained grey-green, while buttons,
# dropdowns, focus states and other interactive accents keep their stronger green.
...
AUTO_FARMLAND_THEME = {
    # Passive window and card surfaces use a cooler, subtly brighter grey-green.
    # Interactive controls retain the existing green palette below.
...
# as centered passive background instead of stretching cards and controls.
...
# A checkbox group followed by another control receives a separate transition
# gap without carrying the larger separation to the bottom of the card.
...
# The slab selector uses the same compact four-column tile layout as Auto
# Light. Wrapped labels use a slightly tighter line advance, while slab artwork
# moves downward without changing the tile dimensions. The bottom clamp keeps
# differently sized icons clear of the lower rounded border.
...
# The Bedrock visual mapping groups some numeric growth values into
# the same crop texture. Wheat uses all eight textures; the other standard crops
# use four visual stages across the eight supported Bedrock growth values.
...
def _set_wrapped_text(control, label):
    """Update one responsive description and reflow it at its current width."""
    try:
...
# =========================
# LOCAL AMULET RESOURCE ARTWORK
# =========================
class AmuletCropIconCache:
    """Render crop growth previews from Amulet's local vanilla resource cache.

    The plugin never downloads artwork or requires separately installed assets.
    Missing resources or Pillow support are nonfatal, and crop selectors remain
    fully usable as labeled text controls.
    """

    TEXTURE_LIMIT = 4 * 1024 * 1024

    def __init__(self):
        """Initialize lazy crop-resource discovery and bitmap caches."""
        self._loaded = False
    ...
    def _java_cache_root(cls):
        """Return Amulet's materialized Java resource-pack cache root."""
        return (
    ...
    def _find_texture_path(self, texture_name):
        """Return the first bounded crop texture from the ordered roots."""
    ...
    def _load_texture(self, texture_name):
        """Load and memoize one crop texture, including failed lookups."""
    ...
    def _stem_tint(state):
        """Return Minecraft's age-based stem tint approximation for state 0 to 7."""
    ...
    def _tint_stem(cls, image, state):
        """Apply Minecraft's growth-state tint to a stem texture."""
        image = image.convert("RGBA")
    ...
    def _source_sprite(self, crop, state):
        """Return the source crop sprite for one crop and growth state."""
        state = max(0, min(7, int(state)))
        ...
        # Java's stem_growth0 through stem_growth7 models expose the first
        # 2, 4, 6 ... 16 texture rows while increasing the plant's model height.
        # Rebuild that same progression on a full 16 x 16 transparent canvas.
    ...
    def ensure_loaded(self):
        """Discover local crop artwork once and report whether any is usable."""
        if self._loaded:
        ...
        # Load one representative state for each supported crop. Remaining
        # states stay lazy so opening the plugin remains lightweight.
        for crop in (*STANDARD_CROPS, "Melon Stem", "Pumpkin Stem"):
            self._source_sprite(crop, 0)
        return self.available_count() > 0

    def available_count(self):
        """Return the number of supported crops with available local artwork."""
        if not self._loaded:
            self.ensure_loaded()
        count = 0
        for crop in (*STANDARD_CROPS, "Melon Stem", "Pumpkin Stem"):
    ...
    def get_bitmap(self, crop, state, size):
        """Return one fixed-state crop bitmap or wx.NullBitmap when unavailable."""
        self.ensure_loaded()
        try:
            state = max(0, min(7, int(state)))
        ...
        # Scale the complete 16 x 16 canvas rather than cropping transparent
        # pixels. This preserves the real relative plant height across states.
...
class AmuletSlabIconCache:
    """Render slab item previews from Amulet's local Java resource cache.

    Item JSON resolves the displayed block model, and the block-model chain
    supplies the top and side textures. Missing models, textures, or Pillow
    support are nonfatal; the selector automatically remains text-only.
    """

    MODEL_JSON_LIMIT = 2 * 1024 * 1024
    TEXTURE_LIMIT = 4 * 1024 * 1024
    # Slab models are rendered above their displayed size, then reduced with
    # high-quality filtering. This keeps Minecraft texture detail recognizable
    # while smoothing the diagonal edges of the isometric geometry.
    BASE_ICON_SIZE = 96

    def __init__(self):
        """Initialize lazy slab-resource discovery and bitmap caches."""
        self._loaded = False
        self._resource_roots = []
        self._base_icons = {}
        self._bitmap_cache = {}
    ...
    def _texture_relative_path(texture_reference):
        """Convert a namespaced texture reference to a relative PNG path."""
    ...
    def _resolve_item_textures(self, item_id):
        """Resolve the top and side textures used by one slab item model."""
    ...
    def _load_texture(self, texture_reference):
        """Load one bounded slab texture from the ordered resource roots."""
    ...
    def _first_animation_frame(texture):
        """Return the first square frame from a packed animated texture."""
    ...
    def _render_slab_icon(cls, top_texture, side_texture):
        """Render an anti-aliased isometric lower-slab item.

        Source textures stay at their native 16-pixel resolution. The geometry
        is drawn on a four-times supersampled canvas and reduced once with
        Lanczos filtering, which smooths diagonal model edges without replacing
        the recognizable Minecraft texture pattern with a soft redraw.
        """
        ...
        # The standard slab model uses the lower half of the side texture.
    ...
    def _content_bbox(image):
        """Return the visible alpha bounds for one rendered slab icon."""
        try:
    ...
    def _fit_icon_to_size(cls, image, size, padding=0):
        """Fit visible slab artwork into a square transparent canvas."""
        image = image.convert("RGBA")
        size = max(8, int(size))
    ...
    def ensure_loaded(self):
        """Discover local slab artwork once and report whether any is usable."""
        if self._loaded:
    ...
    def available_count(self):
        """Return the number of selectable slabs with available local artwork."""
        self.ensure_loaded()
        return len(self._base_icons)

    def get_bitmap(self, choice, size, padding=0):
        """Return a popup-grid bitmap for one slab choice."""
        self.ensure_loaded()
        choice = str(choice)
        size = max(8, int(size))
        padding = max(0, int(padding))
    ...
    def get_compact_bitmap(self, choice, size):
        """Return a compact closed-selector bitmap for one slab choice."""
        self.ensure_loaded()
        choice = str(choice)
        size = max(12, int(size))
        key = (choice, size)
...
class ModernButton(wx.Control):
    ...
    def __init__(
        ...
        # separate makes the final active state fully owned by this control.
        self._busy = False
    ...
    def Enable(self, enable=True):
        ...
        # Auto Farmland already manages their availability and busy state itself.
...
class ModernScrollViewport(wx.Panel):
    """Manual clipped viewport for custom Auto Farmland scrolling areas.
    ...
    def GetContentWindow(self):
        """Return the child panel that owns all scrollable controls."""
        return self._content

    def SetContentSizer(self, sizer):
        """Attach and lay out the sizer used by the scrollable child panel."""
...
class ModernIconChoiceOption(ModernChoiceOption):
    ...
    def _on_paint(self, _event):
        ...
        if bitmap_ok:
            ...
            # All visual choices in this popup are slabs, so one shared
            # downward offset keeps their top faces clear of wrapped labels.
            # Retain a small lower margin in case a bitmap is taller.
            maximum_icon_y = max(
                0.0,
                float(
                    size.height
        ...
        for line in lines:
            ...
            # Tighten only the effective distance to a following wrapped line.
            # A single-line label is unchanged because this value is not reused.
            line_y += max(
                1.0,
                text_height
                + _dip(self, 1)
...
class ModernChoice(wx.Panel):
    ...
    def _refresh_selected_icon(self):
        ...
        if self._show_icons and choice:
            # Render directly from the original item texture. The compact item preview is requested directly from the provider so it is
            # scaled only once for the closed field.
...
class CropToggleTile(wx.Control):
    """Selectable crop tile with fixed-state or range-aware live artwork."""
    ...
    def GetLabel(self):
        """Return the crop label through the standard control-style API."""
        return self._label

    def SetLabel(self, label):
        """Update the displayed crop identity and repaint its preview."""
        self._label = str(label)
        self.Refresh(False)

    def SetValue(self, value):
        """Set the tile's checked state without emitting an event."""
        self._value = bool(value)
        self.Refresh(False)

    def GetValue(self):
        """Return the tile's current checked state."""
        return bool(self._value)

    def SetSelected(self, selected):
        """Set the exclusive-selector highlight state."""
        self.SetValue(selected)

    def SetShowIcons(self, show=True):
        """Show or hide crop artwork without changing the selection."""
        self._show_icons = bool(show)
        self.Refresh(False)

    def SetPreviewStates(self, minimum, maximum=None, range_mode=False):
        """Update the growth-state previews painted inside this tile."""
    ...
    def Enable(self, enable=True):
        """Update native enabled state, cursor, and painted appearance."""
        result = super().Enable(enable)
    ...
    def _on_paint(self, _event):
        ...
        # Keep the growth-state badge near the lower edge of the preview area
        # so it identifies the state without covering the crop artwork.
...
class CropExclusiveSelector(wx.Panel):
    """Four-tile single-crop selector with the wx.Choice value API."""
    ...
    def FindString(self, value):
        """Return the index of one crop label or wx.NOT_FOUND."""
        try:
            return self._choices.index(str(value))
        except ValueError:
            return wx.NOT_FOUND
    ...
    def SetSelection(self, selection):
        """Select one crop tile by index without emitting a choice event."""
        selection = int(selection)
    ...
    def GetSelection(self):
        """Return the selected crop index."""
        return self._selection

    def GetStringSelection(self):
        """Return the selected crop name or an empty string."""
        if 0 <= self._selection < len(self._choices):
            return self._choices[self._selection]
        return ""

    def SetPreviewStates(self, minimum, maximum=None, range_mode=False):
        """Update every tile's fixed and random growth-state previews."""
        for tile in self._tiles:
            tile.SetPreviewStates(minimum, maximum, range_mode)

    def SetShowIcons(self, show=True):
        """Show or hide artwork for every crop tile."""
        for tile in self._tiles:
            tile.SetShowIcons(show)

    def Enable(self, enable=True):
        """Synchronize enabled state across the selector and all tiles."""
        result = super().Enable(enable)
        for tile in self._tiles:
            tile.Enable(enable)
        return result
...
class AutoFarmlandWindow(wx.Frame):
    """Single modeless Auto Farmland window owned by the Amulet operation host."""
    ...
    def destroy_for_host(self):
        """Allow and perform final destruction when the operation host closes."""
    ...
    def _on_close(self, event):
        """Hide normal user closes while allowing host-requested destruction."""
...
class AutoFarmland(wx.Panel, DefaultOperationUI):
    """Surface-aware farmland planner with a self-themed floating interface."""
    ...
    # Keep the settings viewport bounded so its full content height does not
    # become the floating window's minimum. The content remains reachable
    # through the manual scrollbar while the footer and report console retain
    # their own visible sections, matching Auto Light's layout.
    ...
    def __init__(self, parent, canvas, world, options_path):
        """Build the launcher, floating interface and persistent UI state."""
    ...
    def _on_main_content_root_size(self, event):
        """Update the centered content width whenever the frame client area changes."""
        self._sync_main_content_width()
        event.Skip()

    def _build_floating_ui(self):
        """Build the complete green Auto Farmland floating interface."""
        ...
        # All visible interface sections live in one centered column. Its width
        # is updated from the root client area, while the frame remains free to
        # grow beyond the visual width that is comfortable for the controls.
        ...
        # Match Auto Light's bounded manual viewport. Without this explicit
        # minimum, wx may use the complete crop-card content height as the
        # viewport's best size and place the console below the visible frame.
        ...
        # Crop layout and visual selection card.
        ...
        # Match the established Auto Farmland default rather than the first list item.
        ...
        # Status and primary actions use the same lower-window structure as
        # Auto Light. The report console is a separate root section immediately
        # below this footer rather than part of the settings viewport or footer.
        footer = wx.Panel(
            self._main_content_panel,
            style=wx.BORDER_NONE | wx.FULL_REPAINT_ON_RESIZE | wx.CLIP_CHILDREN,
        )
        ...
        # Independent report-console section. It is a sibling of the footer,
        # exactly as in Auto Light, so hiding it removes the complete section.
        ...
        # Control synchronization and events.
        self._bind_slider_and_text(self.growth_slider, self.growth_box, 0, 7)
        # Random-growth controls use one source-aware handler so each slider and
        # text field updates its matching partner without duplicate event paths.
    ...
    def _position_plugin_window_at_amulet_edge(self):
        """Place the window vertically centered and half-overlapping Amulet's left edge."""
        window = self._plugin_window
        if window is None:
            return
        try:
    ...
    def _update_launcher_status(self):
        """Refresh the compact launcher to reflect floating-window visibility."""
        window = self._plugin_window
    ...
    def _apply_saved_ui_data(self, data):
        """Restore persisted control values and floating-window state."""
        ui = data.get("ui", {}) if isinstance(data, Mapping) else {}
        if not isinstance(ui, Mapping):
            ui = {}
        # Restore console visibility first so the active frame minimum matches
        # the saved layout before the saved window dimensions are applied. This
        # prevents a hidden-console 580-pixel window from being forced to the
        # visible-console 720-pixel minimum during startup.
    ...
    def _toggle_console(self, _event=None):
        """Show or hide the complete report-console section."""
    ...
    def _set_control_tooltip(self, control, text):
        """Bind one custom non-activating tooltip to a control and its children."""
    ...
    def _on_host_destroy(self, event):
        """Close the floating window and release host-owned resources safely."""
        try:
    ...
    # intentionally retained even though ordinary in-file call searches cannot
    # see the framework-owned entry points.
    ...
    def enable(self):
        """Enable Amulet's block-selection behavior for this operation host."""
        self._selection.enable()
    ...
    def _settings_registry(self) -> Mapping[str, wx.Window]:
        """Map stable config keys to the modern user-controlled fields."""
        return {
    ...
    def _control_value(control):
        """Convert one supported modern control into JSON-safe data."""
    ...
    def _apply_settings_data(self, data: Mapping[str, object]) -> None:
        """Apply recognized settings and validated floating-window state."""
    ...
    def _normalize_settings_config_data(
        ...
        *,
        ...
        """Preserve unknown keys while validating settings and UI dimensions."""
    ...
    def _schedule_settings_save(self, event=None) -> None:
        """Debounce automatic settings writes after a control or window changes."""
        try:
    ...
    def _show_plugin_file_action_dialog(
        ...
        """Show the green self-themed plugin-file action picker."""
    ...
    def _migrate_legacy_settings_data(
        ...
        """Map legacy single-crop config keys into the current layout system."""
        migrated = dict(data)
    ...
    # -----------------------------------------------------------------
    # Floating-interface behavior and dependent controls
    # -----------------------------------------------------------------

    def _bind_slider_and_text(self, slider, text_box, minimum, maximum) -> None:
        """Keep one modern slider and numeric text field synchronized."""
        def on_slider(event):
    ...
    def _on_ui_setting_changed(self, event) -> None:
        """Refresh dependent controls and queue the updated settings."""
        self._update_ui_visibility()
        self._update_growth_description()
        self._schedule_settings_save(event)
    ...
    def _on_random_growth_changed(self, event) -> None:
        """Synchronize the random-growth slider or text field that changed."""
        try:
    ...
    def _update_crop_previews(self) -> None:
        """Refresh visible crop artwork for the active growth state or range."""
    ...
    def _update_growth_description(self) -> None:
        """Refresh growth descriptions and all state-dependent crop previews."""
    ...
    def _selected_crop_controls(self) -> Tuple[Tuple[str, CropToggleTile], ...]:
        """Return standard crop tiles in their stable displayed order."""
        return (
    ...
    # -----------------------------------------------------------------
    # Selection, block and plant helpers
    # -----------------------------------------------------------------
    @staticmethod
    ...
    # -----------------------------------------------------------------
    # Surface scanning and crop layout
    # -----------------------------------------------------------------
    ...
    # -----------------------------------------------------------------
    # Plan assembly, validation and writing
    # -----------------------------------------------------------------
    def _build_plan(
        self,
        boxes: Sequence[BoxTuple],
    ...
    # -----------------------------------------------------------------
    # Operation settings and Amulet wrapper
    # -----------------------------------------------------------------
