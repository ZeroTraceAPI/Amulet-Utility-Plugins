"""Blocks to Storage v2.0.0.0 plugin for Amulet Map Editor.

Purpose:
- Scan the selected Minecraft Bedrock Edition world area.
- Count exportable blocks and their resulting inventory items.
- Clear selected blocks while preserving protected blocks such as bedrock.
- Store collected items in chests, barrels or shulker boxes.
- Support separated groups, spacing, item-frame labels and nested packing.
- Present the complete control set in a self-themed floating window.

Navigation:
1. Imports and optional NBT compatibility
2. Purple custom UI theme and storage icon resources
3. Painted controls, scrolling, selectors and dialogs
4. Layered Windows dropdowns and tooltips with portable fallbacks
5. Floating window, launcher and window lifecycle
6. Core conversion tables and embedded display names
7. Scan-identity recovery, UI state and warning helpers
8. Logging, reports, selection and direction helpers
9. Block conversion, inventory and NBT helpers
10. Settings persistence and managed-file operations
11. Conversion rules and Amulet diagnostics
12. Installed language data and ABC ordering
13. Item packing, chunk access, scanning and clearing
14. Storage and item-frame placement
15. Main operation wrapper and Amulet export registration
"""
...
# Passive surfaces carry a restrained purple tint. Interactive controls use a
# stronger purple so the plugin remains distinct without bathing every card in
# saturated color.
...
# Tooltips use the same bright purple outline as an active / open dropdown.
# Sharing the exact theme value keeps the tooltip visually tied to the custom
# controls instead of maintaining a separate near-match colour.
...
# Collapsible content is inset slightly and paints its own rounded shell.
# Matching the child radius to the outer card prevents the square content
# panel from covering the smooth lower corners while a section is open.
...
def _set_checkbox_group_bottom_spacing_visible(item, visible):
    """Show or hide one explicit group-end spacer with its dynamic group."""
    if item is None:
        return
    try:
...
# The storage-container selector contains exactly three choices. Shulker color
# selectors use the same four-column visual arrangement as Auto Light and keep
# four rows visible at once; the seventeenth option remains reachable through
...
STORAGE_ICON_FALLBACK_SPECS = {
    # Chest and shulker-box items use Minecraft's special entity-backed
    # renderers instead of ordinary block-model parents. Barrel uses a normal
    # cube-bottom-top model, but needs its real top texture rather than applying
    # the side texture to every face. These local-cache specifications preserve
    # the recognizable inventory appearance without online assets.
...
# Approximate Minecraft dye colors used only for the visual shulker swatches.
# Persisted values and generated world data continue to use the existing exact
# lowercase color identifiers.
SHULKER_COLOR_RGB = {
    "default": (116, 81, 132),
    "white": (222, 222, 214),
    "orange": (216, 127, 51),
...
# Shulker color dropdowns show readable labels while preserving the original
# lowercase identifiers used by saved settings, resource-cache keys, generated
# item names, and Bedrock block-state values.
...
def _shulker_color_value(value):
    """Return one stable lowercase shulker color identifier."""
...
def _shulker_color_label(value):
    """Return the readable dropdown label for one shulker color value."""
    return SHULKER_COLOR_LABELS[_shulker_color_value(value)]


def _storage_icon_choice_key(choice):
    """Map readable shulker labels back to resource-cache icon keys."""
    text = str(choice)
...
class AmuletStorageIconCache:
    """Resolve optional storage artwork from Amulet's local caches.
    ...
    def _resolve_model_spec(self, item_id):
        """Resolve one item model to a supported generated or cube render specification."""
    ...
    def _model_specs(self):
        """Build render specifications for every configured storage choice."""
        specs = {}
        special_kinds = {"chest_entity", "barrel_block", "shulker_entity"}
        for choice, item_id in STORAGE_ICON_ITEM_IDS.items():
            # Prefer the explicit local-cache renderer for special entity items
            # and for the barrel's separate top / side textures. Generic model
            # resolution remains available for any additional ordinary choices.
    ...
    def _render_flat_icon(cls, texture):
        """Center one flat source texture on the selector icon canvas."""
    ...
    def _render_prism_icon(
        ...
        *,
        ...
        """Render one anti-aliased isometric prism from three source faces."""
        render_size = cls.BASE_ICON_SIZE * 4
        geometry_scale = render_size / 128.0
        # Keep each source face at its native pixel dimensions. Chest and
        # shulker entity faces are not all square, and forcing them to 16 x 16
        # distorts lids, seams, latches, and overlapping shell geometry.
    ...
    def _render_chest_icon(cls, texture):
        """Render a familiar closed-chest item from the local entity sheet.

        The chest sheet contains both inward-facing and outward-facing cuboid
        faces. Use the exterior lid face, the clean lock-facing front, and the
        matching latch face so the closed chest does not show the recessed lid
        underside or the rear-face UV notch.
        """
        texture = texture.convert("RGBA")
        if texture.width < 56 or texture.height < 43:
            return cls._render_flat_icon(texture)

        # Lid cuboid: the second 14 x 14 square is the exterior upper face.
        # The first square is the darker inward / underside face and should not
        # be visible on a closed inventory-style chest.
        lid_top = texture.crop((28, 0, 42, 14))
        lid_west = texture.crop((0, 14, 14, 19))
        lid_front = texture.crop((14, 14, 28, 19))

        # Base cuboid: use the clean front face rather than the opposite face,
        # whose central dark notch belongs to the rear-facing UV arrangement.
        base_west = texture.crop((0, 33, 14, 43))
        base_front = texture.crop((14, 33, 28, 43))

        # The matching front of the narrow 2 x 4 latch cuboid.
    ...
    def _render_barrel_icon(cls, side_texture, top_texture):
        """Render the closed barrel with its real top and side textures."""
        return cls._render_prism_icon(
    ...
    def _render_shulker_icon(cls, texture):
        """Render a closed shulker box from its entity texture sheet.

        The lid is 12 pixels high and overlaps the upper four pixels of the
        eight-pixel base. The transparent lower lid pixels are intentional, so
        the base must be composed behind the lid before projection. This
        reproduces the recessed shell seam instead of painting the particle
        texture onto a generic cube.
        """
        texture = texture.convert("RGBA")
    ...
    def _add_shulker_color_swatches(self):
        """Add compact isometric color indicators for shulker choices."""
        if Image is None or ImageDraw is None:
            return
        ...
        # The storage-container selector uses the title-cased shulker label.
    ...
    def available_count(self):
        """Return the number of storage icons currently available."""
        self.ensure_loaded()
        return len(self._base_icons)

    def get_bitmap(self, choice, size, padding=0):
        """Return a square popup bitmap for one storage choice."""
        self.ensure_loaded()
...
class ModernButton(wx.Control):
    ...
    def __init__(
        ...
        # separate makes the final accent state fully owned by this control.
        self._busy = False
    ...
    def Enable(self, enable=True):
        ...
        # Blocks to Storage already manages their availability and busy state itself.
...
class ModernScrollViewport(wx.Panel):
    """Manual clipped viewport for custom Blocks to Storage scrolling areas.
...
class ModernIconChoiceOption(ModernChoiceOption):
    ...
    def _on_paint(self, _event):
        ...
        if bitmap_ok:
            ...
            # Every storage choice is a full block. Apply the same seven-pixel
            # downward shift to chests, barrels, the undyed shulker box, and
            # every dyed shulker box while retaining a small lower margin.
            maximum_icon_y = max(
                0.0,
                float(
                    size.height
...
class ModernChoicePopup(wx.Frame):
    ...
    def rebuild(self, owner, choices, selection):
        """Rebuild text rows or the optional storage icon grid."""
    ...
    def show_for(self, owner):
        ...
        # Match Auto Light's selector placement: center the popup on the
        # owning field, open below when space allows, otherwise place it above,
        # and clamp the completed window to the active monitor work area.
...
class ModernChoice(wx.Panel):
    ...
    def GetIconPopupColumns(self):
        """Return the configured number of columns for this icon popup."""
        return max(1, int(self._icon_columns))

    def GetIconPopupVisibleRows(self):
        """Return the preferred visible row count, or None for automatic height."""
    ...
    def UsesIconPopup(self):
        """Return whether every displayed choice has usable local artwork."""
    ...
    def _refresh_selected_icon(self):
        ...
        if self._show_icons and choice:
            # Render directly from the stored source. Reusing an already
            # scaled bitmap can soften small texture details a second time.
    ...
    def _on_paint(self, _event):
        ...
        # rounded focus border around small or narrow selector artwork.
...
def _show_dark_message(
    ...
    *unused_position,
    ...
    """wx.MessageBox-compatible wrapper using the Blocks to Storage palette."""
...
def _apply_rounded_child_shape(window, radius=10):
    """Clip a child panel so its opaque background cannot square off a card.

    RoundedPanel controls normally rely on painting alone because they live
    inside an already opaque interface. Collapsible content is different: its
    child window can cover the anti-aliased corners painted by the parent card.
    Windows therefore receives a native rounded window region, while other
    platforms use the shared wx.Region fallback.
    """
    try:
        size = window.GetClientSize()
        width = int(size.GetWidth())
...
class ModernSectionButton(ModernButton):
    """Custom collapsible-section header with a wx.ToggleButton value API."""
...
class ModernFileCheckRow(RoundedPanel):
    """Selectable managed-file row with a wrapped status description."""
...
class DarkMultiChoiceDialog(wx.Dialog):
    """Resizable self-themed multi-file selector used by Manage Plugin Files."""
...
class PluginClassName(wx.Panel, DefaultOperationUI):
    ...
    # reviewed and verified Amulet-assisted fallbacks may affect unresolved
    ...
    # verified built-in state-aware resolvers. Advanced diagnostics can opt into
    ...
    # Readable UI labels; conversion helpers retain stable internal values.
    ...
    # these names have been verified as valid item frame display items.
    ...
    # skipped instead of risking empty or ghost storage entries. These generic
    # names remain unmapped unless their exact Minecraft Bedrock Edition item
 ...
 'yellow_harness': ('item.yellow_harness.name', 'Yellow Harness')}
    ...
    # verified from the
    # installed en_US.lang file and runtime-verified across direct blocks,
    ...
    # available for additional edge cases, but add blocks only when they are
    # known to create invalid storage or item-frame entries.
    ...
    def __init__(
        ...
        """Build the launcher, floating interface and unchanged operation state."""
        ...
        # Tooltips are suspended while an operation is starting or running.
        # Amulet may move focus and temporarily interrupt normal enter / leave
        # events during canvas.run_operation, so explicit suspension prevents a
        # delayed tooltip from appearing away from the plugin and remaining open.
    ...
    def _build_launcher_ui(self):
        """Build the compact compatibility host shown in Amulet's Operations tab."""
        outer = wx.BoxSizer(wx.VERTICAL)
        self.SetSizer(outer)
        margin = 6
    ...
    def _create_card(self, parent, title, subtitle=None):
        """Create one rounded purple-tinted settings card."""
        card = RoundedPanel(
            parent,
    ...
    def _create_collapsible_card(self, parent, label, subtitle=None, expanded=True, parent_sizer=None):
        """Create a rounded card whose content keeps a persisted expanded state."""
        card = RoundedPanel(
            parent,
            background=BLOCKS_TO_STORAGE_THEME["passive_surface"],
            border=BLOCKS_TO_STORAGE_THEME["passive_border"],
        ...
        # A normal wx.Panel paints a square opaque background. When expanded,
        # that square background can cover the rounded card's lower corners.
        # Use a borderless rounded child with the same fill as the card so its
        # clipping and painting follow the outer geometry.
        ...
        # Painting a rounded rectangle is not enough for an opaque child window:
        # its native rectangular region can still cover the parent's smooth
        # corners. Keep the actual child-window region rounded as it resizes.
        ...
        # The inset exposes the outer card border, while the rounded child keeps
        # the expanded section's lower corners smooth and fully opaque.
        card_sizer.Add(
            content_panel,
            0,
            wx.LEFT | wx.RIGHT | wx.BOTTOM | wx.EXPAND,
    ...
    def _build_labeled_choice(self, parent, sizer, label, choices, tooltip, icon_provider=None, show_icons=False):
        """Build one labeled choice row and return its container and control."""
    ...
    def _add_checkbox(self, parent, sizer, attribute, label, value, tooltip):
        ...
        # Match Auto Light's compact checkbox rhythm. The smaller shared
        # control gap keeps consecutive options visually grouped without
        # changing the check box control itself.
    ...
    def _build_floating_ui(self):
        """Build the complete purple Blocks to Storage floating interface."""
        ...
        # Performance and Safety retain their existing collapsible state keys.
        ...
        # This group-end spacer belongs to the conditional installed-language
        # controls below. It is hidden with those controls so an inactive
        # option group does not leave extra space between ordinary checkboxes.
        ...
        # Diagnostics and controlled conversion simulations, including the
        # nested Advanced section.
        ...
        # Footer and independent report console.
        footer = wx.Panel(
            self._main_content_panel,
            style=wx.BORDER_NONE | wx.CLIP_CHILDREN,
        )
    ...
    def _bind_integer_slider_and_text(self, slider, text_box, minimum, maximum):
        """Keep a ModernSlider and numeric ModernTextField synchronized."""
    ...
    def _position_plugin_window_at_amulet_edge(self):
        """Position the window near Amulet's left edge while keeping it on-screen."""
        window = self._plugin_window
        if window is None:
            return
        try:
    ...
    def _on_plugin_window_size(self, event):
        """Persist the normal floating-window size after layout has settled."""
        try:
            event.Skip()
        except Exception:
            pass
    ...
    def _apply_saved_window_size(self, data):
        """Restore the saved floating-window size within the current display bounds."""
        window = self._plugin_window
    ...
    def _update_floating_minimum_size(self, resize_if_needed=False):
        """Apply the correct minimum window size for the console visibility state."""
        window = self._plugin_window
        if window is None:
            return
        minimum_width, minimum_height = self._current_floating_minimum_size()
    ...
    def _apply_saved_console_visibility(self, data):
        """Restore the saved report-console visibility before final sizing."""
        visible = True
        try:
    ...
    def _finish_operation_ui(self, status_label, generation):
        if self._destroying or generation != self._operation_ui_generation:
            return
        self._operation_running = False
        # Remove any help window that may have been queued before Amulet took
        # over, then resume normal hover behavior only after processing ends.
        self._set_tooltips_suspended(True)
        try:
    ...
    def _set_tooltips_suspended(self, suspended):
        """Suspend or resume all custom help bubbles.

        Amulet's operation dialog and canvas worker can interrupt the normal
        pointer leave sequence. Hiding both tooltip types and rejecting pending
        delayed callbacks while processing prevents orphaned bubbles.
        """
    ...
    # Amulet calls these lifecycle methods outside this module. They remain
    # explicit because ordinary in-file call searches cannot see framework-owned
    # entry points.
    ...
    def _configure_tooltips(self) -> None:
        """Configure native fallback timing; primary help uses custom hints."""
        try:
    ...
    def _get_scan_identity_source(self, alias: str) -> str:
        ...
        reviewed and runtime-verified. Found Entries keeps priority over the
    ...
    def _on_simulate_missing_display_name_changed(self, _) -> None:
        """
        Refreshes the simulated-missing-entry field when the diagnostic
    ...
    def _update_option_visibility(self) -> None:
        """Apply the established option dependencies to the modern controls."""
    ...
    def _normalize_language_alias(self, value: str) -> str:
        """
        Converts language keys and diagnostic aliases into the resolver's alias form.
        """
    ...
    def _settings_control_registry(self):
        """Return the persisted setting names and their corresponding controls."""
        control_names = (
            "storage_choice",
    ...
    def _collect_current_settings_config(self) -> dict:
        """Collect current control values into the settings configuration structure."""
        settings = {}
    ...
    def _load_settings_config_data(self, path: Path) -> Optional[dict]:
        """Read and validate the active Blocks to Storage settings file."""
        try:
            path = Path(path)
            if not path.is_file():
                return None
    ...
    def _apply_settings_config_data(self, data: dict) -> None:
        """Apply validated settings and interface state to the current controls."""
        ...
        try:
            ...
            # Visibility must be restored before size so hidden-console 580px
            # windows are not clamped to the visible-console 720px minimum.
    ...
    def _merge_settings_config_data(self, existing: Optional[dict]) -> dict:
        """Merge current settings into saved data while preserving unknown keys."""
    ...
    def _bind_settings_persistence_events(self) -> None:
        """Bind controls that schedule automatic settings persistence."""
        custom_handled = {
    ...
    def _stop_pending_settings_save(self) -> None:
        """Stop a queued automatic save before an explicit file action."""
        pending = self._settings_config_save_call
        self._settings_config_save_call = None
        if pending is not None:
            try:
    ...
    def _reset_settings_to_defaults(self) -> bool:
        """Apply current defaults and intentionally rewrite the active config."""
    ...
    def _show_plugin_file_action_dialog(
        ...
        """Show the themed action dialog for managed plugin files."""
    ...
    def _parse_conversion_entries_file(self, path: Path) -> Dict[str, str]:
        ...
        The supported format is intentionally conservative:
        minecraft:source_block=minecraft:item_name

        Comments, blank lines and unrecognized section headers are ignored.
    ...
    def _log_amulet_translator_validation_probe(self) -> None:
        """
        Reports cached translator capabilities and bounded block round-trip checks.
        """
    ...
    def _get_simulated_missing_item_alias(self) -> str:
        """
        Returns the one embedded item alias ignored by the diagnostic
        simulation.
        """
    ...
    def _should_ignore_embedded_display_name(self, item_name: str) -> bool:
        """
        Returns whether the diagnostic simulation should skip embedded
        resolution.
        """
    ...
    def _resolve_language_display_name(
        ...
        for candidate in candidates:
            ...
            # replaces a trusted embedded entry outside the diagnostic
            # simulation.
    ...
    def _get_fallback_display_sort_key(self, item_name: str) -> str:
        """
        Returns the verified ABC sort key without language resolution.
        """
        item_name = str(item_name)
    ...
    def _scan_selection(self):
        ...
        for x, y, z in self._iter_selected_positions():
            ...
            # state-sensitive suffix check for the same string.
    ...
    def _run_export(self, _):
        """Validate warnings and start the unchanged Amulet operation wrapper."""
        if self._operation_running:
            return
        # Dismiss any visible or delayed help bubble before Amulet changes
        # focus, captures the pointer, or opens its operation progress UI.
        self._set_tooltips_suspended(True)
        self._clear_log()
        self._reset_report()
        self._update_option_visibility()
    ...
    def _run_export_operation_guarded(self):
        """Run the operation and restore the custom UI after preflight failures."""
        try:
