...
# Conditional sources
# may be inactive in the saved world state and are handled separately below.
LIGHT_STRENGTH = {
    "torch": 14,
    "soul_torch": 10,
    "copper_torch": 14,
...
class AutoLighting(wx.Panel, DefaultOperationUI):
    ...
    Major behaviors:
    - Supports exact legacy radius detection and optional calculated light coverage
    ...
    # These values keep the upper settings viewport and built-in console
    # independently adjustable. The settings viewport remains scrollable, so
    # reducing its height does not hide controls permanently.
    ...
    def __init__(self, parent, canvas, world, options_path):
        ...
        # The built-in console mirrors the Blocks to Storage report console.
        # Each run replaces the visible report and keeps a complete in-memory
        # copy that can be saved explicitly as a UTF-8 text file.
        self._report_lines = []
        ...
        # Scrollable layout so the UI stays usable as controls grow. The
        # viewport uses a fixed preferred height instead of consuming every
        # spare vertical pixel above the console.
        ...
        # STATUS LABEL
        # =========================
        # A single compact line reports both the outcome and elapsed time. The
        # detailed timing breakdown remains available in the built-in console.
        ...
        # =========================
        # OPERATION AND REPORT CONTROLS
        # =========================
        # The settings remain in the scrolled upper panel while the operation
        # button, report control, and console stay visible below it.
    ...
    def _get_settings_config_path(self):
        """Returns the stable local path used for Auto Light.config."""
    ...
    def _apply_settings_config_data(self, data):
        """Applies recognized values while retaining unknown future data."""
    ...
    def _show_settings_action_dialog(self, actions):
        """Shows a centered settings-action picker with stable description space."""
    ...
    def _is_conditional_source_active(self, block):
        ...
        state cannot be proven locally. The default UI option can instead treat
        these blocks as potentially lit.
        """
    ...
    def _get_calculated_light_level(self, x, y, z, light_buckets):
        """Returns only the strongest Smart Coverage level at a position."""
    ...
    def _get_block_if_available(self, x, y, z, dim, plat, ver):
        ...
        Placement checks can cross a chunk boundary while testing floors,
    ...
    def _run_operation(self, _):
        ...
        # Spread mode preserves the older minimum circular distance of 1.
